...
`retrying` is kept in this filter for backwards compatibility (a pre-existing row in that status is still claimable), but as of Superthread 98 nothing writes that status anymore - see below.
