...
**Behavior**: loads all scenes (ordered), calls `ensureVisualDescriptions` to establish the cohesive plan, then sends the target scenes' `{ scene_id, title, visual_description }` to the script-category LLM. The system prompt frames each image as the coherent first frame a video will animate from and injects the shared `visual_style_anchor` for cross-scene consistency. Expects JSON `[{ scene_id, prompt }]` back.
...
**Input**: `{ scene_id?: string }`; outputs labeled `video_prompt_scene_<sceneId>`. `output_summary: { prompt_count }`.
...
4. **Graceful fallback to text-only** when: the provider is not vision-capable, no `scene_image_*` exists, the image fetch fails, or the vision call throws (it retries once without the image). See `ScriptParams.images` in [Providers](/backend/providers).
