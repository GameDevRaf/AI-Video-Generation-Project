...
Throwing anywhere fails the job immediately; users can retry explicitly from the UI after correcting a key or input. See [job-queue-and-worker.md](/backend/job-queue-and-worker).
...
3. Generates **one call per scene** (so each prompt can be conditioned on its own frame). The system prompt asks for a concise motion prompt (under 80 words) that animates *from* the first frame, consistent with the shared style anchor. The user turn also includes the scene's target shot length as a **rounded whole-second value** (`~Ns`) so the requested action pacing matches the clip length. Returns plain text (no JSON).
...
| `image_path` | `string?` | Canonical path to the scene's private first-frame image; the worker creates a fresh signed URL before calling a provider |
| `image_url` | `string?` | Legacy/external-provider image URL compatibility field; internal assets should use `image_path` |
...
5. Insert a `job_outputs` row labeled `final_export_mp4` whose `metadata` is a **manifest** (version 2): mode, total duration, per-scene entries with times and durable media paths. The export API adds fresh signed URLs when serving the manifest to an authenticated user.
...
| Function | Purpose |
|---|---|
| `getLatestSceneVideos / getLatestSceneImages / getSceneAudioPaths (projectId, sceneIds)` | latest-wins maps `sceneId → storage_path` from job_outputs labels |
| `getVoiceTrackPath(projectId)` | newest `voice_track` storage path or null |
