...
description: "Shared server helpers for crypto, ffmpeg, media uploads, and Supabase access."
---
...
SHA-256 of the required `API_KEY_ENCRYPTION_SECRET` → the 32-byte AES key. **Consequence:** changing the encryption secret breaks decryption of all previously saved provider keys (users must re-enter them). The service-role key is not used for encryption.

## `server/utils/storage.ts`

`createSignedAssetUrl(supabase, storagePath, expiresIn?)` creates a one-hour signed URL for the private `assets` bucket. The storage path is the durable identifier; signed URLs are generated only for a response or an internal provider/ffmpeg download and must not be persisted.
...
⚠️ Import `shared/` **by relative path only** (see [common-pitfalls.md](/guides/common-pitfalls)) - the standalone worker doesn't resolve Nuxt aliases.
