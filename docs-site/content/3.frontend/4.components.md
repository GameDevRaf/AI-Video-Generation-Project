...
When `Skip Video Gen` is enabled, the whole scene card is deliberately dimmed so it is clear the video controls are out of the active export path and the final MP4 will use scene images instead.
**Props**: `scene`, `prompt`, `videoUrl`, `imageUrl`, `generationPrompt?`, `dataLoaded?`, `isActive`, `dimmedBySkipVideoGen?`, `generating`, `generatingPrompt?`, `uploading?`, `providerError?`.
