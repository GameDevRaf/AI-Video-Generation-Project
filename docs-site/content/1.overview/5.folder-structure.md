...
├── public/                 # Static files served as-is (robots.txt, if present)
...
Generated Playwright output is ignored by Git and must never be committed.
...
├── plugins/api-fetch.ts    # Keeps the runtime fetch helper available globally
...
│   ├── storage.ts          #   private-bucket signed URL helper
...
└── worker/                 # The background job processor
    ...
    ├── loop.ts             #   poll → claim → dispatch → explicit-failure logic (3s interval)
    ├── lib/
