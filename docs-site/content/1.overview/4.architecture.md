---
title: "Architecture"
description: "Understand the application architecture, job queue, and generation pipeline."
---
...
> Prefer pictures? [diagrams.md](/overview/diagrams) has rendered Mermaid flowcharts of everything below - a system overview, a detailed module map, and the user flow.
...
4. Results are written to the **`job_outputs`** table - text outputs go into `metadata.content`; binary files (images/audio/video) are uploaded to the private Supabase Storage `assets` bucket and referenced by `storage_path`.
5. Status → `completed` (with an `output_summary`), or on a thrown error → `failed` with `error_message`; retries are explicit and create a new job row.
...
Per stage, what happens (details in [job-handlers.md](/backend/job-handlers)):
...
**"Latest wins" convention:** regeneration never overwrites - it inserts a *new* `job_outputs` row with the same `label` (e.g. `scene_image_<sceneId>`). Every reader sorts by `created_at DESC` and takes the newest per scene. See [conventions.md](/database/conventions).
...
Adding a provider touches catalog + adapter + registry. Guide: [add-a-provider.md](/guides/add-a-provider).
...
- Users' AI provider keys are stored **AES-256-GCM encrypted** in `api_keys`; the encryption key is derived from the required `API_KEY_ENCRYPTION_SECRET`. Secrets are only ever decrypted inside the worker; API routes never return them.
- Media paths are stored durably in database rows. Authenticated API routes create one-hour signed URLs when media is requested, and the worker creates signed URLs when it needs to read internal assets.
...
| `shared/config/videoFormat.ts` | The single hardcoded output format (9:16, 1080×1920, 180 s max) |
