...
description: "Trace one generation request through the frontend, API, database, and worker."
---
...
**If any step throws** (bad key, provider outage, JSON parse error), `loop.ts` catches it and marks the job `failed` with the error message. The UI's explicit Retry action creates a new job row after the user corrects the key or input.
...
| `image_prompt` / `video_prompt` | one job covers all scenes (or one with `input.scene_id`); both first ensure the project's cohesive **visual descriptions** exist (generate-once) | one text output per scene, label `image_prompt_scene_<id>` / `video_prompt_scene_<id>` |
...
**The prompt chain.** `image_prompt` and `video_prompt` no longer read raw `script_text` directly. Both first call `ensureVisualDescriptions` (`server/worker/lib/visualDescriptions.ts`), which plans all scenes together once for cross-scene visual consistency and stores `visual_description_scene_<id>` + a shared `visual_style_anchor`. Image prompts describe each scene's first frame; video prompts describe motion from that frame and, for vision-capable providers (`anthropic`/`openai`/`gemini`), attach the generated `scene_image_<id>` to the LLM call (text-only fallback otherwise). See [Job Handlers](/backend/job-handlers).
