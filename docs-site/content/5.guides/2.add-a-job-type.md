...
description: "Step-by-step guide for adding a new generation pipeline job type."
---
...
Follow the shared skeleton (see [job-handlers.md](/backend/job-handlers)):
...
- Pick output labels per [conventions.md](/database/conventions) - per-scene assets embed the scene id in the label.
