...
description: "Step-by-step guide for adding a safe authenticated HTTP endpoint."
---
...
export default defineEventHandler(async (event) => {
  ...
  // 5. Sign paths before returning them - never persist or expose public URLs
  return Promise.all((data ?? []).map(async output => ({
    ...output,
...
4. **Latest-wins reads** for job_outputs: order newest-first, dedupe per scene label ([conventions.md](/database/conventions)).
...
```ts
const data = await $fetch<{ label: string; storage_path: string; url: string }[]>('/api/music', {
...
Then hit it in the browser devtools Network tab or with the dev server running. Finally, document it in [api-routes.md](/backend/api-routes).
