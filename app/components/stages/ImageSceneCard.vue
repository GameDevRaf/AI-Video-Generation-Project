<template>
  <div class="flex flex-col gap-3 p-4 rounded-xl border border-white/10 bg-white/3">
    ...
    <button
      type="button"
      class="w-32 mx-auto aspect-[9/16] rounded-lg overflow-hidden bg-white/5 border border-white/8 flex items-center justify-center disabled:cursor-default"
      :disabled="!imageUrl"
    ...
    <div class="flex flex-col gap-2">
      <div class="flex items-center gap-2">
        ...
        <div class="relative flex-1 min-w-0">
          ...
          <button
            ...
            class="w-full px-2.5 py-1.5 bg-white/8 border border-white/10 text-white rounded-lg text-xs font-medium hover:bg-white/12 transition-colors disabled:opacity-40 whitespace-nowrap"
            @click="$emit('generate-image', scene.id, localPrompt)"
          >
            <span v-if="generating" class="flex items-center justify-center gap-1.5">
      ...
      <button
        :disabled="uploading"
        class="w-full px-2.5 py-1.5 border border-white/10 text-gray-300 rounded-lg text-xs font-medium hover:bg-white/5 transition-colors disabled:opacity-40 whitespace-nowrap"
